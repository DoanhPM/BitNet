...
def ensure_file(path, hint):
    if not os.path.exists(path):
        logging.error(f"Missing required file or directory: {path}")
        logging.error(hint)
        sys.exit(1)

def ensure_command(name, hint):
    if shutil.which(name) is None:
        logging.error(f"{name} is not available in PATH.")
        logging.error(hint)
        sys.exit(1)
...
def setup_gguf():
    ensure_file(
        os.path.join("3rdparty", "llama.cpp", "gguf-py"),
...
def compile():
    ...
    ensure_command("cmake", "Install CMake or create the env with: conda create -n bitnet-cpp python=3.9 cmake ninja")
    ensure_command("clang", "On Windows, run from Developer PowerShell for VS 2022 with the C++ Clang component installed.")
    _, arch = system_info()
    if arch not in COMPILER_EXTRA_ARGS.keys():
        logging.error(f"Arch {arch} is not supported yet")
        exit(0)
    logging.info("Compiling the code using CMake.")
    run_command(["cmake", "-B", "build", "-DCMAKE_BUILD_TYPE=Release", *COMPILER_EXTRA_ARGS[arch], *OS_EXTRA_ARGS.get(platform.system(), []), "-DCMAKE_C_COMPILER=clang", "-DCMAKE_CXX_COMPILER=clang++"], log_step="generate_build_files")
...
def parse_args():
    ...
    parser.add_argument("--skip-model-prepare", action="store_true", help="Only install gguf, generate kernels, and compile; do not download/convert/check a model")
